...
export function CreateMarker() {
  const { items, hintPath, newPath, markers, newStaion, stationPath, stopCreate, setStationPath, setGeometry, setItems, setNewStation, setHintPath, setNewPath, setMarkers } = useAdminStore()
  ...
  // const { stations, setStations } = useBusStore();
    function MyComponent() {
        const map = useMapEvents({
            ...
            setStationPath(copy)
            // 생성한 마커와 ID를 상태에 추가
            setMarkers([...markers, { marker: 1, title: `경로${markers.length+1}`, drag: true, lat: lat, lng: lng, stop: false }]);
            console.log('클릭 좌표:', lat, lng);
            console.log(newMarker)
            console.log(markers)
            copy = JSON.parse(JSON.stringify(items))
            copy['routes'].push({id: `경로${markers.length}`, title: `경로${markers.length+1}`, status: false, lat: lat, lng: lng, stop: false, isExist: false})
            setItems(copy)
            handleOpen()
            }
        });
      ...
      >
        <Card className="mx-auto w-full max-w-[24rem]">
          ...
          <CardBody className="flex flex-col gap-4">
            <Input label="마커이름" value={markerName} size="lg" onChange={(e) => setMarkerName(e.target.value)} />
          </CardBody>
          <CardFooter className="pt-0">
            ...
            copy['routes'][copy['routes'].length - 1].title = markerName;
            copy['routes'][copy['routes'].length - 1].id = markerName;
            let flag = 0;
            console.log(copy)
        ...
        {/* <MyComponent></MyComponent> */}
          {
            stopCreate  ? 
          markers.map(({ marker, title, drag, stop }, index) => (
            stop &&
              <Marker
                key={index}
